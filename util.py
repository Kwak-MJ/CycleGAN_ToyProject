...
def save(ckpt_dir, netG_a2b, netG_b2a, netD_a, netD_b, optimG, optimD, epoch):
    ...
    torch.save({'netG_a2b': netG_a2b.state_dict(), 'netG_b2a': netG_b2a.state_dict(),
               ...
               "%s/electric2person_epoch%d.pth" % (ckpt_dir, epoch))
...
def add_blur(img, type="bilinear", opts=None):
    ...
    dst = resize(img, output_shape=(
        sz[0] // opts[0], sz[1] // opts[0], sz[2]), order=order)
...
def image2patch(src, nimg, npatch, nmargin, datatype="tensor"):
    ...
    src = np.pad(src, ((nmargin[0], nmargin[0]), (nmargin[1], nmargin[1]),
                 (nmargin[2], nmargin[2]), (nmargin[3], nmargin[3])), 'reflect')
    dst = np.zeros((nsmp * npatch[0], npatch[1],
                   npatch[2], npatch[3]), dtype=np.float32)
        ...
        for j in range(0, nset[1]):
            for k in range(0, nset[2]):
                for q in range(0, nset[3]):
                    ...
                    pos = [nset[3] * nset[2] * nset[1] * i +
                           nset[2] * nset[1] * j + nset[1] * k + q]
...
def patch2image(src, nimg, npatch, nmargin, datatype="tensor", type="count"):
    ...
    dst = np.zeros([nimg_zp[0], nimg_zp[1], nimg_zp[2],
                   nimg_zp[3]], dtype=np.float32)
    wgt = np.zeros([nimg_zp[0], nimg_zp[1], nimg_zp[2],
                   nimg_zp[3]], dtype=np.float32)

    i_img = [np.arange(nmargin[0], nimg_zp[0] - nmargin[0]).astype(np.int32)[:, np.newaxis, np.newaxis, np.newaxis],
             np.arange(nmargin[1], nimg_zp[1] - nmargin[1]
                       ).astype(np.int32)[:, np.newaxis, np.newaxis],
             np.arange(nmargin[2], nimg_zp[2] - nmargin[2]
        ...
        for j in range(0, nset[1]):
            for k in range(0, nset[2]):
                for q in range(0, nset[3]):
                    ...
                    src_ = src[pos, :, :, :]
                    dst[i_, j_, k_, q_] = dst[i_, j_, k_, q_] + \
                        src_[crop[0], crop[1], crop[2], crop[3]] * wgt_
