...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, transform=None, task=None, data_type='both'):
        ...
        if os.path.exists(self.data_dir_a):
            ...
            lst_data_a = [f for f in lst_data_a if f.endswith(
                'jpg') | f.endswith('jpeg') | f.endswith('png')]
            lst_data_a.sort()
        else:
            lst_data_a = []
        ...
        if os.path.exists(self.data_dir_b):
            ...
            lst_data_b = [f for f in lst_data_b if f.endswith(
                'jpg') | f.endswith('jpeg') | f.endswith('png')]
            lst_data_b.sort()
        else:
            lst_data_b = []
    ...
    def __getitem__(self, index):
        ...
        if self.data_type == 'a' or self.data_type == 'both':
            data_a = plt.imread(os.path.join(
                self.data_dir_a, self.lst_data_a[index]))[:, :, :3]
        ...
        if self.data_type == 'b' or self.data_type == 'both':
            ...
            data_b = plt.imread(os.path.join(
                self.data_dir_b, self.lst_data_b[index]))[:, :, :3]
